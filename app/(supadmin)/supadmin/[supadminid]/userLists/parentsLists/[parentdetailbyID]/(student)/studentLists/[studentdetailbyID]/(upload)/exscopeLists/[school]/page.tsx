...
//                         href={`/supadmin/${supadminId}/userLists/parentsLists/${ParentID}/studentLists/${StudentID}/exscopeLists/${SchoolName}/${grades.school_grade}`}
//                     >
...
// export default ExScope_Gradebysupadmin
...
const ExScope_Gradebysupadmin = () => {
  ...
  if (
    !data ||
    !Array.isArray(data) ||
    !data.every((item) => typeof item.school_grade === "number")
  ) {
    return (
      <div className="bg-red-50 text-red-600 p-4 rounded-lg">
        無效的年級資料格式
      ...
      <div className="flex flex-col space-y-4">
        {data.map((grades) => (
          <Link
            ...
            href={`/supadmin/${supadminId}/userLists/parentsLists/${ParentID}/studentLists/${StudentID}/exscopeLists/${SchoolName}/${grades.school_grade}`}
          >
            {gradeMapping[grades.school_grade.toString()] || `未知年級 (${grades.school_grade})`}
          </Link>
        ))}
      </div>
    </div>
