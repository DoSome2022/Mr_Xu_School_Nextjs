...
// }
// const ScoreLists_Year_Quarter_SubjectbySupadmin = () => {
//     const params = useParams<{parentdetailbyID : string ; studentdetailbyID : string ;  grade : string ; year: string; quarter:string; supadminId:string;}>();
...
//                     href={`/supadmin/${supadminId}/userLists/parentsLists/${ParentID}/studentLists/${StudentID}/scoreLists/${Grade}/${Year}/${Quarter}/${subject.school_subject}`}
//                 >
...
// export default ScoreLists_Year_Quarter_SubjectbySupadmin
...
const ScoreLists_Year_Quarter_SubjectbySupadmin = () => {
  ...
  // 驗證路由參數
  if (!supadminId || !ParentID || !StudentID || !Grade || !Year || !Quarter) {
    return (
  ...
  // 獲取科目資料
  const { data: subjectsData, error: subjectsError, isLoading: subjectsLoading } = useSWR<StudentSchoolSubject[]>(
    `${apiUrl_DJANGO}/api/School_data/schoolsubjects?studentId=${StudentID}&grade=${encodeURIComponent(Grade)}&year=${encodeURIComponent(Year)}&quarter=${encodeURIComponent(Quarter)}`,
    fetcher,
  ...
  // 錯誤處理
  if (subjectsError || studentError) {
    return (
      <div className="bg-red-50 text-red-600 p-4 rounded-lg">
        錯誤：無法載入資料 - {(subjectsError || studentError)?.message || "未知錯誤"}
      </div>
    );
  }

  // 載入中
  if (subjectsLoading || studentLoading) {
  ...
  if (
    ...
    !subjectsData.every((item) => typeof item.school_subject === "string" && item.school_subject)
  ) {
    return (
      <div className="bg-red-50 text-red-600 p-4 rounded-lg">
        無效的科目資料格式
  ...
  // 過濾科目：若 API 不支援查詢參數，可根據需要添加前端過濾
  const filteredSubjects = subjectsData; // 若 API 已過濾，則直接使用
  ...
  return (
    <div className="container mx-auto px-4 py-6 bg-blue-50 min-h-screen">
      ...
      <h2 className="text-2xl font-semibold text-blue-600 mb-4">
        {studentData[0].name} 的 {gradeMapping[Grade]} {Year} {quarterMapping[Quarter]} 科目列表
      </h2>

      {filteredSubjects.length === 0 && <div className="text-gray-600 p-4">無科目資料</div>}
      ...
      <div className="flex flex-col space-y-4">
        {filteredSubjects.map((subject) => (
          <Link
            ...
            href={`/supadmin/${supadminId}/userLists/parentsLists/${ParentID}/studentLists/${StudentID}/scoreLists/${encodeURIComponent(Grade)}/${encodeURIComponent(Year)}/${encodeURIComponent(Quarter)}/${encodeURIComponent(subject.school_subject)}`}
          >
            科目 {subject.school_subject}
          </Link>
        ))}
