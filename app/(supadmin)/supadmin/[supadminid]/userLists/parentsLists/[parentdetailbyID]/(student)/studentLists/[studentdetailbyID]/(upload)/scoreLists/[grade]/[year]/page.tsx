...
// const ScoreLists_Year_Quarterbysupadmin = () => {

//     const params = useParams<{parentdetailbyID : string ; studentdetailbyID : string ;  grade : string ; year: string;supadminId: string;}>();
...
//                     href={`/supadmin/${supadminId}/userLists/parentsLists/${ParentID}/studentLists/${StudentID}/scoreLists/${Grade}/${Year}/${quarters.school_quarter}`}
//                 >
...
// export default ScoreLists_Year_Quarterbysupadmin
...
const ScoreLists_Year_Quarterbysupadmin = () => {
  ...
  // 驗證路由參數
  if (!supadminId || !ParentID || !StudentID || !Grade || !Year) {
    return (
  ...
  // 獲取季度資料
  const { data: quartersData, error: quartersError, isLoading: quartersLoading } = useSWR<StudentSchoolQuarter[]>(
    `${apiUrl_DJANGO}/api/School_data/schoolquarters?studentId=${StudentID}&grade=${encodeURIComponent(Grade)}&year=${encodeURIComponent(Year)}`,
    fetcher,
  ...
  // 錯誤處理
  if (quartersError || studentError) {
    return (
      <div className="bg-red-50 text-red-600 p-4 rounded-lg">
        錯誤：無法載入資料 - {(quartersError || studentError)?.message || "未知錯誤"}
      </div>
    );
  }

  // 載入中
  if (quartersLoading || studentLoading) {
  ...
  if (
    !quartersData ||
    !Array.isArray(quartersData) ||
    !quartersData.every((item) => typeof item.school_quarter === "string" && ["1", "2", "3", "4"].includes(item.school_quarter))
  ) {
    return (
      <div className="bg-red-50 text-red-600 p-4 rounded-lg">
        無效的季度資料格式
  ...
  // 過濾季度：若 API 不支援查詢參數，可根據需要添加前端過濾
  const filteredQuarters = quartersData; // 若 API 已過濾，則直接使用
  ...
  return (
    <div className="container mx-auto px-4 py-6 bg-blue-50 min-h-screen">
      ...
      <h2 className="text-2xl font-semibold text-blue-600 mb-4">
        {studentData[0].name} 的 {gradeMapping[Grade]} {Year} 季度列表
      </h2>

      {filteredQuarters.length === 0 && <div className="text-gray-600 p-4">無季度資料</div>}
      ...
      <div className="flex flex-col space-y-4">
        {filteredQuarters.map((quarter) => (
          <Link
            ...
            href={`/supadmin/${supadminId}/userLists/parentsLists/${ParentID}/studentLists/${StudentID}/scoreLists/${encodeURIComponent(Grade)}/${encodeURIComponent(Year)}/${encodeURIComponent(quarter.school_quarter)}`}
          >
            {quarterMapping[quarter.school_quarter]}
          </Link>
        ))}
      </div>
    </div>
