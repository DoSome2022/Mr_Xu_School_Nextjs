...
//                     href={`/supadmin/${supadminId}/userLists/parentsLists/${ParentID}/studentLists/${StudentID}/schooltimetableLists/${SchoolName}/${Grade}/${Year}/${quarters.school_quarter}`}
//                 >
...
const SchoolTimeTableLists_Grade_Year_Quarterbysupadmin = () => {
  ...
  if (
    !data ||
    !Array.isArray(data) ||
    !data.every((item) => typeof item.school_quarter === "string" && ["1", "2", "3", "4"].includes(item.school_quarter))
  ) {
    return (
      <div className="bg-red-50 text-red-600 p-4 rounded-lg">
        無效的季度資料格式
  ...
  return (
    <div className="container mx-auto px-4 py-6 bg-blue-50 min-h-screen">
      ...
      <h2 className="text-2xl font-semibold text-blue-600 mb-4">
        {SchoolName} 年級 {Grade} 學年 {Year} 季度列表
      </h2>

      {data.length === 0 && <div className="text-gray-600 p-4">無季度資料</div>}
      ...
      <div className="flex flex-col space-y-4">
        {data.map((quarters) => (
          <Link
            ...
            href={`/supadmin/${supadminId}/userLists/parentsLists/${ParentID}/studentLists/${StudentID}/schooltimetableLists/${encodeURIComponent(SchoolName)}/${encodeURIComponent(Grade)}/${encodeURIComponent(Year)}/${encodeURIComponent(quarters.school_quarter)}`}
          >
            季度 {quarters.school_quarter}
          </Link>
        ))}
