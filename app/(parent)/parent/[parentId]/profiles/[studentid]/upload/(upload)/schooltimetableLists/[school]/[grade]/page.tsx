...
const SchoolTimeTableLists_Grade_Year = () => {
  ...
  const fetcher = (url: string, init?: RequestInit):Promise<StudentYear[]>  => fetch(url, init).then((res) => res.json());
  const apiUrl = process.env.NEXT_PUBLIC_API_URL_For_DJANGO|| "http://127.0.0.1:8000"
    ...
    return (
        <>
            ...
            {data.map((year) => (
                <div key={year.school_year}> {/* 添加 key 属性 */}
                    <br />
                    <Link
                        className="text-stone-950 hover:text-gray-700"
                        href={`/parent/${ParentID}/profiles/${StudentID}/upload/schooltimetableLists/${SchoolName}/${Grade}/${year.school_year}`}
                    >
                        年份 : {year.school_year}
                    </Link>
                    <br />
