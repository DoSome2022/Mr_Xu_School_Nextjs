...
export default function ClassRoomCalendar() {
  ...
  const filteredClassRooms = filteredRooms
    ...
    .filter((room) => room.Class.length > 0);
  ...
  return (
    <div className="bg-white/10 backdrop-blur-md rounded-lg p-6 shadow-lg">
      {error ? (
        <div className="p-4 text-cyan-200">錯誤: {error.message}</div>
      ) : isLoading ? (
                              ...
                              <div key={cls.id} className="text-white">
                                ...
                                {visibleRooms[`${room.id}-${cls.id}`] && (
                                  <div>
                                    ...
                                    <button
                                      className="rounded bg-gray-500 px-2 py-1 text-white hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-all duration-300"
                                      onClick={() => toggleVisibility(`${room.id}-${cls.id}`)}
                                    >
                                      {visibleRooms[`${room.id}-${cls.id}`] ? "顯示簡化" : "顯示詳情"}
                                    </button>
