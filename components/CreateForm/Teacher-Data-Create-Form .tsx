...
interface TeacherDataCreateFormProps {
  TeacherId: { teacherdetailbyID: string };
}
...
const Teacher_Data_Create_Form = ({ TeacherId }: TeacherDataCreateFormProps) => {
  ...
  const teacher_data_register_form_onSubmit = (values: z.infer<typeof Teacher_Data_Create_Schema>) => {
    console.log("-- 老師數據創建輸入 -- : ", values, "-- End --");
    setError("");
    setSuccess("");
    startTransition(() => {
      create_Teacher_Data(values).then((data) => {
        setError(data?.error);
        setSuccess(data?.success);
      });
    });
  ...
  return (
    <Form {...teacher_data_register_form}>
      <form onSubmit={teacher_data_register_form.handleSubmit(teacher_data_register_form_onSubmit)} className="space-y-6">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-white font-medium">科目</FormLabel>
              <FormControl>
                <SWR_School_Subject_checkbox teacher_data={teacher_data_register_form} />
              </FormControl>
            ...
            <FormItem hidden>
              <FormControl>
                <Input
                  ...
                  value={TeacherId.teacherdetailbyID}
                  type="text"
                  className="border-0 bg-white text-[#e7915b]"
                />
              </FormControl>
              <FormMessage className="text-cyan-200" />
            </FormItem>
        ...
        <FormField
          control={teacher_data_register_form.control}
          name="teacher_time_work_id"
          render={({ field }) => (
            <FormItem hidden>
              <FormControl>
                <Input {...field} disabled type="text" className="border-0 bg-white text-[#e7915b]" />
              </FormControl>
              <FormMessage className="text-cyan-200" />
            </FormItem>
          )}
