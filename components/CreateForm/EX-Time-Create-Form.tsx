...
//         ex_time_create_form.setError("img", { message: "僅支持 JPG、JPEG、PNG 或 PDF 格式" });
//         return;
//       }
//       if (file.size > 5 * 1024 * 1024) {
//         setError("文件大小不能超過 5MB");
//         ex_time_create_form.setError("img", { message: "文件大小不能超過 5MB" });
//         return;
//       }
...
//           setSuccess("考試時間表創建成功");
//           toast.success("考試時間表創建成功");
//           // 刷新 SWR 緩存
//           mutate(`/api/School_Lists`); // 假設 SWR 鍵，根據實際調整
//           // 重定向
//           router.push(`/admin/schoolLists/${SchoolId}/extimeLists/`);
//           ex_time_create_form.reset();
//           setPreviewImage(null);
//         } else {
//           setError(data?.error || "創建考試時間表失敗");
//           toast.error(data?.error || "創建考試時間表失敗，請檢查輸入數據或文件格式");
//         }
//       });
//     });
//   };
...
//       <h2 className="text-xl font-semibold text-gray-700">上傳考試時間表 - {data[0]?.school_name || "學校"}</h2>
...
//             render={({ field }) => (
//               <FormItem>
//                 <FormLabel className="text-gray-700 font-medium">上傳考試時間表文件</FormLabel>
//                 <FormControl>
//                   <Input
//                     type="file"
//                     disabled={isPending}
...
const EX_Time_Create_Form = ({ SchoolId, data }: EX_Time_Create_FormProps) => {
  ...
  //       setError("文件類型無效，請上傳 JPG、PNG 或 PDF 格式");
  //       ex_time_create_form.setError("img", { message: "文件類型無效，請上傳 JPG、PNG 或 PDF 格式" });
  //       return;
  //     }
  //     if (file.size > 5 * 1024 * 1024) {
  //       setError("文件大小不能超過 5MB");
  ...
  //       ex_time_create_form.setValue("img", base64String);
  //       setPreviewImage(base64String);
  //       setUploadedFileName(file.name); // 記錄原始文件名
  //     };
  //     reader.readAsDataURL(file);
  //   }
  // };
const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
  if (e.target.files && e.target.files.length > 0) {
    ...
    reader.onload = () => {
      ...
      ex_time_create_form.setValue("img", base64String);
      setPreviewImage(base64String);
      setUploadedFileName(file.name); // 設置為 string
    };
    reader.readAsDataURL(file);
  }
};
const ex_time_create_form_onSubmit = (values: z.infer<typeof Ex_timetable_Create_Schema>) => {
  ...
  setSuccess("");
  startTransition(() => {
    createExTimeTable({ ...values, originalFileName: uploadedFileName }).then((data) => {
      if (data?.success === "true" && data.data) {
        setSuccess("考試時間表創建成功");
        toast.success("考試時間表創建成功");
        mutate(`/api/School_Lists`);
        router.push(`/admin/schoolLists/${SchoolId}/extimeLists/`);
        ex_time_create_form.reset();
        setPreviewImage(null);
        setUploadedFileName(undefined); // 重置為 undefined
      } else {
        setError(data?.error || "創建考試時間表失敗");
        toast.error(data?.error || "創建考試時間表失敗，請檢查輸入數據或文件格式");
      }
