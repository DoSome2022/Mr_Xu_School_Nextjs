// // components/CreateForm/TimeTemplate-Create-Form.tsx
// "use client";
...
// import { toast } from "sonner"; // 引入 sonner 用於錯誤提示
...
// // 輔助函數：將日期字符串標準化為 YYYY-MM-DD（避免時區偏移）
// const normalizeDateString = (dateStr: string): string => {
//   const date = new Date(dateStr);
//   console.log("time : ",`${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`)
...
//       timetemplate_create_form.setValue("weekdays", weekdays.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()));
//     } else {
...
//                         <DatePicker
//                           multiple
//                           value={value ? value.map((day) => normalizeDateString(day.date)) : []}
...
//           <Button type="submit" disabled={!isDateRangeSelected}>
//             提交
//           </Button>
//         </form>
//       </Form>
...
const TimeTemplate_Create_Form = () => {
  ...
  const fetcher = (...args: Parameters<typeof fetch>): Promise<CourseLesson[]> =>fetch(...args).then((res) => res.json());
                  ...
                  <div>
                    <Controller
                      ...
                      render={({ field: { onChange, value } }) => (
                        <DatePicker
                          multiple
                          value={value ? value.map((day) => day.date) : []} // 直接使用 day.date 字符串
                          minDate={dayStart && !isNaN(new Date(dayStart).getTime()) ? new Date(dayStart) : undefined}
                          maxDate={dayEnd && !isNaN(new Date(dayEnd).getTime()) ? new Date(dayEnd) : undefined}
                          onChange={(dates: DateObject[]) => {
                            setSelectedDays(dates);
