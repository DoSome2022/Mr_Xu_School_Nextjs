  ...
  const onSubmit = (values: z.infer<typeof student_school_timetable_Create_Schema>) => {
    ...
    startTransition(() => {
      createStudentSchoolTimetable(values).then((data) => {
        if (data?.data) {
          setSuccess("學生學校時間表創建成功");
          toast.success("學生學校時間表創建成功");
          router.push(`/admin/userLists/parentsLists/${parentId}/studentLists/${studentId}/schooltimetableLists/`);
          form.reset();
          setPreviewImage(null);
        } else {
          setError(data?.error || "創建學生學校時間表失敗，請檢查輸入數據或文件格式");
          toast.error(data?.error || "創建學生學校時間表失敗，請檢查輸入數據或文件格式");
        }
      }).catch((error) => {
        console.log("Unexpected error: ", error, "-- End --");
        setError("創建學生學校時間表失敗，發生未預期的錯誤");
        toast.error("創建學生學校時間表失敗，發生未預期的錯誤");
      });
    });
  };
