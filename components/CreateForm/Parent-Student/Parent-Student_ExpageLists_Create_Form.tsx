...
//     const parent_student_expagelists_create_form = useForm<z.infer<typeof parent_student_ex_pager_create_schema>>({
...
//                 parent_student_expagelists_create_form.setValue("img", base64String);
//                 parent_student_expagelists_create_form.setValue("originalFileName", file.name);
//             };
//             reader.readAsDataURL(file);
//         }
//     };

//     const parent_student_expagelists_create_form_onSubmit = (values: z.infer<typeof parent_student_ex_pager_create_schema>) => {
//         console.log("Upload started:", values);
//         startTransition(async () => {
//             try {
//                 await createparentstudentexpager(values);
//                 parent_student_expagelists_create_form.reset();
//                 setFileBase64("");
//                 setOriginalFileName("");
//             } catch (error) {
...
//                 if (result[0]?.name) {
//                     parent_student_expagelists_create_form.setValue("student_name", result[0].name);
//                 }
//             };
//             fetchStudentData();
//         }
...
//     return(
//         <>
// <Form {...parent_student_expagelists_create_form}>
//                 <form
//                     onSubmit={parent_student_expagelists_create_form.handleSubmit(parent_student_expagelists_create_form_onSubmit)}
//                     className="space-y-6"
//                 >
                
//                 <div className="space-y-4">
//                 <FormField
//                     control={parent_student_expagelists_create_form.control}
//                     name="name"
//                     render={({ field }) => (
//                 <FormItem>
...
import { toast } from "sonner"; // 導入 sonner 用於提示
import { mutate } from "swr"; // 導入 SWR 的 mutate 用於刷新緩存
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 10 * 1024 * 1024) { // 限制 10MB
        parent_student_expagelists_create_form.setError("img", { message: "文件大小不能超過 10MB" });
        return;
      }
      setOriginalFileName(file.name);
      const reader = new FileReader();
    ...
    startTransition(async () => {
      try {
        ...
        if (result.success === "true" && result.data) {
          toast.success("上傳成功");
          // 刷新 SWR 緩存（假設 API 路徑為 /api/Parents_Student/...，調整為您的實際 SWR 鍵）
