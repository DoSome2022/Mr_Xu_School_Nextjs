...
const Parent_Student_ExscopeLists_Create_Form = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 10 * 1024 * 1024) {
        parent_student_exscopelists_create_form.setError("img", { message: "文件大小不能超過 10MB" });
        return;
      }
      setOriginalFileName(file.name);
      const reader = new FileReader();
  ...
  const parent_student_exscopelists_create_onSubmit = (values: z.infer<typeof parent_student_ex_scope_create_schema>) => {
    startTransition(async () => {
      try {
        const result = await createParentStudentExScope(values);
        if (result.success === "true" && result.data) {
          toast.success("上傳成功");
          // 刷新 SWR 緩存
          await mutate(`/api/Parents_Student/Parents_Student_Lists_detail_data_by_id/${StudentID}`);
          // 重定向
          router.push(`/parent/${ParentID}/profiles/${StudentID}/upload/exscopeLists`);
          parent_student_exscopelists_create_form.reset();
          setFileBase64("");
          setOriginalFileName("");
        } else {
