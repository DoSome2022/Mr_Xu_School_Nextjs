...
const Parent_Student_SchooltimetableLists_Create_Form = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 10 * 1024 * 1024) {
        parent_student_schooltimetablelists_create_form.setError("img", { message: "文件大小不能超過 10MB" });
        return;
      }
      setOriginalFileName(file.name);
      const reader = new FileReader();
  ...
  const parent_student_schooltimetablelists_create_onSubmit = (values: z.infer<typeof parent_student_school_timetable_create_schema>) => {
    startTransition(async () => {
      try {
        const result = await createParentStudentSchoolTimetable(values);
        if (result.success === "true" && result.data) {
          toast.success("上傳成功");
          // 刷新 SWR 緩存
          await mutate(`/api/Parents_Student/Parents_Student_Lists_detail_data_by_id/${StudentID}`);
          // 重定向
          router.push(`/parent/${ParentID}/profiles/${StudentID}/upload/schooltimetableLists`);
          parent_student_schooltimetablelists_create_form.reset();
          setFileBase64("");
          setOriginalFileName("");
        } else {
  ...
  useEffect(() => {
    if (StudentID) {
      const fetchStudentData = async () => {
        ...
        if (result[0]?.school) {
          parent_student_schooltimetablelists_create_form.setValue("school", result[0].school);
        }
      };
      fetchStudentData();
    }
