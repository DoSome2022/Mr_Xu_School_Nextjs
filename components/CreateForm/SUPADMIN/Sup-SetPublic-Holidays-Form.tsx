...
import { SupcreatePublic_holiday } from "@/actions/supadmin/Create-PublicHoliday";
import { Suppublic_holiday_create_Schema } from "@/actions/supadmin/Create-PublicHoliday/schema";



const Public_Holidays_Formbysupadmin = () => {
    const [ isPending , startTransition ] = useTransition();
        const param = useParams();
        console.log("param : ", param ,"-- End --");
        const supadminid = param?.supadminid as string;
        console.log("supadminid :", supadminid);
    const publicholidays_create_form = useForm<z.infer<typeof Suppublic_holiday_create_Schema>>({
            resolver: zodResolver(Suppublic_holiday_create_Schema),
            defaultValues:{
        ...
        // 提交表單
    const publicholidays_create_form_onSubmit = (values : z.infer<typeof Suppublic_holiday_create_Schema>) => {
        console.log("--  create publicholidays -- : ", values ,"-- End --");

        startTransition(() => {
            SupcreatePublic_holiday(values);
        })
    }
...
export default Public_Holidays_Formbysupadmin