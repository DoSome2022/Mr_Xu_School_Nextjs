...
// import { SupUpdate_Student } from "@/actions/supadmin/Update-Student";
...
// const Student_Update_Formbysupadmin = () => {
...
//   const form = useForm<z.infer<typeof SupStudent_Update_Schema>>({
//     resolver: zodResolver(SupStudent_Update_Schema),
//     // defaultValues: {
//     //   name: "",
//     //   school: "",
//     //   grade: 0,
...
//         student_id: studentData.student_id, // 確保與路由參數一致
//         studentId: studentId,
...
// export default Student_Update_Formbysupadmin;
...
const Student_Update_Formbysupadmin = () => {
  ...
  // 驗證學校資料並過濾無效數據
  const validSchoolsData = schoolsData?.filter((item) => item.id && item.school_name) || [];

  if (!validSchoolsData.length) {
    console.log("validSchoolsData : ", validSchoolsData, "-- End --");
    return (
      <div className="bg-red-50 text-red-600 p-4 rounded-lg">
        無有效的學校資料
      </div>
    ...
    <div className="bg-white p-6 rounded-lg shadow-md max-w-lg mx-auto">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          />
          <Button
            disabled={isPending || validSchoolsData.length === 0}
            type="submit"
            className="w-full bg-blue-600 hover:bg-blue-700 text-white"
          >
            {isPending ? "提交中..." : "更新學生資料"}
