...
import { SupEx_pager_Create_Schema } from "@/actions/supadmin/Create-Ex_pager/schema";
import { useParams } from "next/navigation";
import { SupcreateExPager } from "@/actions/supadmin/Create-Ex_pager";
...
const EX_Pager_Create_Form_bysupadmin = ({SchoolId , data}: EX_Pager_Create_FormProps) =>{
    ...
    const ex_pager_create_form = useForm<z.infer<typeof SupEx_pager_Create_Schema>>({
        resolver: zodResolver(SupEx_pager_Create_Schema),
        defaultValues:{
...
const ex_pager_create_form_onSubmit = (values: z.infer<typeof SupEx_pager_Create_Schema>) => {
  console.log("-- create ex pager -- : ", values, "-- End --");
  setError("");
  setSuccess("");
  startTransition(() => {
    SupcreateExPager(values).then((data) => {
      setError(data?.error);
      setSuccess(typeof data?.success === "string" ? data?.success : data?.success ? "考試試卷創建成功" : undefined);
    });
  });
};
...
export default EX_Pager_Create_Form_bysupadmin
