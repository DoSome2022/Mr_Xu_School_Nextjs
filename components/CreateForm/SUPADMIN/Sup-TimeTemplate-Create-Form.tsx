...
const SupTimeTemplateCreateForm = () => {
  ...
  const { data: courseData } = useSWR("/api/course_data/courselessons", fetcher);
  const [isPending, startTransition] = useTransition();
  ...
  useEffect(() => {
    ...
    timeTemplateCreateForm.setValue("publicHoliday", publicHolidays, { shouldValidate: true });
  ...
  useEffect(() => {
    if (!courseData || courseData.length === 0) {
      timeTemplateCreateForm.setValue("weekdays", [], { shouldValidate: true });
      return;
    }
    if (dayStart && dayEnd && Object.values(selectedWeekdays).some(Boolean)) {
      const startDate = new Date(dayStart);
      const endDate = new Date(dayEnd);
      ...
      while (currentDate <= endDate) {
        const dayOfWeek = currentDate.getDay();
        if (selectedWeekdays[dayOfWeek + 1]) {
          const course = courseData[courseIndex % courseData.length];
          weekdays.push({
            date: currentDate.toISOString().split("T")[0],
  ...
  }, [dayStart, dayEnd, selectedWeekdays, courseData, watchedStartTime, watchedEndTime, timeTemplateCreateForm]);
  ...
  useEffect(() => {
    if (selectedDays.length > 0) {
      ...
      timeTemplateCreateForm.setValue("days", days, { shouldValidate: true });
    } else {
      timeTemplateCreateForm.setValue("days", [], { shouldValidate: true });
    }
  ...
  const timeTemplateCreateFormOnSubmit = (values: z.infer<typeof timetemplate_create_Schema>) => {
    startTransition(() => {
      createtimetemplate(values).then((result) => {
        if (result?.error) {
          timeTemplateCreateForm.setError("root", { message: result.error });
        } else if (result?.success) {
          timeTemplateCreateForm.reset();
          console.log("創建時間模板成功:", result.success);
        }
      });
    });
  };
  ...
  return (
    <Form {...timeTemplateCreateForm}>
      <form
        onSubmit={timeTemplateCreateForm.handleSubmit(timeTemplateCreateFormOnSubmit)}
        className="space-y-6 max-w-lg mx-auto p-4 bg-white rounded-md shadow"
      >
        <div className="text-red-500">{timeTemplateCreateForm.formState.errors.root?.message}</div>
        ...
        <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
          ...
          ) : GetPublicHolidays.length > 0 && GetPublicHolidays[0].publicHoliday ? (
              ...
              <Controller
                ...
                render={({ field: { onChange, value } }) => (
                  <DatePicker
                    multiple
                    value={value ? value.map((day) => new Date(day.date)) : []}
                    minDate={dayStart ? new Date(dayStart) : undefined}
                    maxDate={dayEnd ? new Date(dayEnd) : undefined}
              ...
              {fields.length > 0 && (
                <div className="mt-4 space-y-3">
                  {fields.map((field, index) => (
                    <div key={field.id} className="border border-[#e7915b] p-3 rounded-lg">
                      <div className="font-medium text-[#e7915b] mb-2">
                        日期: {timeTemplateCreateForm.getValues(`days.${index}.date`)}
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel className="text-sm text-gray-700">開始時間</FormLabel>
                              <FormControl>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel className="text-sm text-gray-700">結束時間</FormLabel>
                              <FormControl>
                                <SWR_Class_Time field={field} />
                              </FormControl>
                              <FormMessage />
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel className="text-sm text-gray-700">課程</FormLabel>
                              <FormControl>
