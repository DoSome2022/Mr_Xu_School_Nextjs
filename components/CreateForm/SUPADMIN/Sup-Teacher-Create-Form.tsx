...
const teacher_register_form_onSubmit = (values: z.infer<typeof SupTeacher_Create_Schema>) => {
    console.log("-- 老師用戶創建輸入 -- : ", values, "-- End --");
    setError("");
    setSuccess("");
    startTransition(() => {
      SupcreateTeacher(values).then((data) => {
        setError(data?.error);
        setSuccess(typeof data?.success === "string" ? data?.success : data?.success ? "教師創建成功" : undefined);
      });
    });
  };
...
<FormError message={error} />
        <FormSuccess
          message={typeof success === "string" ? success : success ? "教師創建成功" : undefined}
        />
        <FormField
          control={teacher_register_form.control}
          name="username"
...
export default Teacher_Create_Formbysupadmin;