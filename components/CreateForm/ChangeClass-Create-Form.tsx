...
//   const ChangeClass_create_form = useForm<z.infer<typeof ChangeClass_Create_Schema>>({
...
//     setError(null);
//     startTransition(() => {
//       createChangeClass(values).then((result) => {
//         if (result?.error) {
...
//   console.log("-- selectedStudent -- :",selectedStudent,"-- Bug --")
...
//                                 )
//                               }
//                               className="border-[#80A8BD] text-[#80A8BD] hover:bg-[#80A8BD] hover:text-white transition-colors duration-300"
//                             >
//                               選擇
//                             </Button>
//                           </td>
...
//                           {student.grade}
//                         </td>
//                         <td className="border border-gray-300 p-3 text-gray-600">無課堂</td>
//                         <td className="border border-gray-300 p-3">
//                           <Button
//                             variant="outline"
//                             size="sm"
...
// components/CreateForm/ChangeClass_Create_Form.tsx
"use client";
...
interface Class {
  id: string;
  classroom: { id: string; room: string }[]; // 更新為物件陣列
  class_lesson: string;
  persons: number;
  teacher: string;
  student: { id: string; name: string }[];
...
const ChangeClass_Create_Form = () => {
  ...
  const [GetStudentData, setGetStudentData] = useState<Student[]>([]);
  const [GetClassData, setGetClassData] = useState<Class[] | null>(null); // 修正類型
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [filter, setFilter] = useState("");
  const [selectedStudent, setSelectedStudent] = useState<{
    name: string;
  ...
  useEffect(() => {
    if (GetClassData && GetClassData.length > 0 && GetClassData[0].class_date) {
      ChangeClass_create_form.setValue("class_date", GetClassData[0].class_date);
    }
  }, [GetClassData, ChangeClass_create_form]);
    ...
    startTransition(() => {
      createChangeClass(values).then((result) => {
        if (result?.error) {
          ...
          setSuccess("調堂記錄創建成功，正在跳轉...");
          setTimeout(() => {
            router.push(`/admin/courseLists/${courseId}/classLists/${values.targetclassId}`);
          }, 2000); // 2秒後跳轉
          ChangeClass_create_form.reset();
          setSelectedStudent(null);
        }
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-gray-700">
        課堂時間: {GetClassData && GetClassData.length > 0 ? GetClassData[0].class_date : "載入中..."}
      </h2>
      <Form {...ChangeClass_create_form}>
        <form
          onSubmit={ChangeClass_create_form.handleSubmit(ChangeClass_create_form_onSubmit)}
