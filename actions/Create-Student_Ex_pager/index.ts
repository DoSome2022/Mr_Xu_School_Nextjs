...
//             }
//         });
//         revalidatePath(`/admin/userLists/parentsLists/${parentId}/studentLists/${student_ex_paper_id}/expageLists/`)
//     } catch (error) {
//         console.log(error)
//     }
...
const handler = async (data: InputType): Promise<ReturnType> => {
  ...
  try {
    // 驗證 student_ex_paper_id 和 school
    const student = await db.student.findUnique({ where: { id: student_ex_paper_id } });
    if (!student) {
    ...
    if (img) {
      ...
      const safeFileName = `${timestamp}-${name}-${student_name}-${school}-${subject}-${grade}-${year}-${quarter}`;
    ...
    // 返回成功結果
    return {
      data: student_ex_paper_Data,
      student_ex_paper_id,
    };
  } catch (error) {
    // 類型縮窄：檢查 error 是否為 Error 實例
    let errorMessage = "無法創建學生考試卷記錄，請檢查輸入數據或文件格式";
    ...
    console.error("創建學生考試卷記錄失敗:", errorMessage);
