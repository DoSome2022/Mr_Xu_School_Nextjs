...
//   const { name, img, school_school_timetable_id, school_name, grade, year, quarter } = data;

//   let school_timetable_Data;

//   try {
//     // 驗證 school_school_timetable_id 和 school_name
//     const school = await db.school.findUnique({ where: { id: school_school_timetable_id } });
//     if (!school) {
...
//       const safeFileName = `${timestamp}-${name}-${school_name}-${grade}-${year}-${quarter}`;
...
//     return {
//       success: "true",
//       data: school_timetable_Data,
//       school_school_timetable_id,
//     };
//   } catch (error) {
//     console.error("創建學校時間表記錄失敗:", error);
//     return { error: "無法創建學校時間表記錄，請檢查輸入數據或文件格式" };
//   }
// };
...
const handler = async (data: InputType): Promise<ReturnType> => {
  ...
  try {
    ...
    let errorMessage = "無法創建學校時間表記錄，請檢查輸入數據或文件格式";
    ...
    console.error("創建學校時間表記錄失敗:", errorMessage);
    return { error: errorMessage };
  } finally {
    revalidatePath(`/admin/schoolLists/${school_school_timetable_id}/timetableLists/`);
  }
};
