...
const handler = async (data: InputType): Promise<ReturnType> => {
  const { name, img, school_ex_scope_id, school_name, grade, quarter, subject } = data;

  let ex_scope_Data;

  try {
    // 驗證 school_ex_scope_id 和 school_name
    const school = await db.school.findUnique({ where: { id: school_ex_scope_id } });
    if (!school) {
