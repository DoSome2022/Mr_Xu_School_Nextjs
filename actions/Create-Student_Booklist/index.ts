...
const handler = async (data: InputType): Promise<ReturnType> => {
  const { studentId, parentId, name, img, student_booklist_id, student_name, grade, year, school } = data;

  let student_booklist_Data;

  try {
    // 驗證 student_booklist_id 和 school
    const student = await db.student.findUnique({ where: { id: student_booklist_id } });
    if (!student) {
    ...
    if (img) {
      ...
      const safeFileName = `${timestamp}-${name}-${student_name}-${school}-${grade}-${year}`;
    ...
    let errorMessage = "無法創建學生書單記錄，請檢查輸入數據或文件格式";
    ...
    console.error("創建學生書單記錄失敗:", errorMessage);
