...
export const weekdaySchema = z.object({
  date: z.string().min(1, { message: "工作日日期不能為空" }),
  start_time: z.string().min(1, { message: "開始時間不能為空" }),
  end_time: z.string().min(1, { message: "結束時間不能為空" }),
  lesson: z.string().min(1, { message: "課程不能為空" }),
});

export const daySchema = z.object({
  date: z.string().min(1, { message: "日期不能為空" }),
  start_time: z.string().min(1, { message: "開始時間不能為空" }),
  end_time: z.string().min(1, { message: "結束時間不能為空" }),
  lesson: z.string().min(1, { message: "課程不能為空" }),
});
