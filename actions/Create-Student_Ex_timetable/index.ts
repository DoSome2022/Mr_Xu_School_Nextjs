...
//             }
//         });
//         revalidatePath(`/admin/userLists/parentsLists/${parentId}/studentLists/${student_ex_timetable_id}/extimeLists`)
//     } catch (error) {
//         console.log(error)
//     }
...
const handler = async (data: InputType): Promise<ReturnType> => {
  ...
  try {
    // 驗證 student_ex_timetable_id 和 school
    const student = await db.student.findUnique({ where: { id: student_ex_timetable_id } });
    if (!student) {
    ...
    let errorMessage = "無法創建學生考試時間表記錄，請檢查輸入數據或文件格式";
    ...
    console.error("創建學生考試時間表記錄失敗:", errorMessage);
